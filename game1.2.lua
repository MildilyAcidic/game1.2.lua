...
local DataStore = DataStoreService:GetDataStore("7qUJkXKd9dfLmvpZ6dxhyyVdBgM5seZm5x5")
...
local function zw(Plr)
	if Plr then
		return (ReplicatedStorage.Values.ZaWarudo.Value and not ReplicatedStorage.Values.ZaWarudo.Owner.Value == Plr)
	else
		return ReplicatedStorage.Values.ZaWarudo.Value
	end
end
...
local function tag(th, p)
	...
	if th:FindFirstChild("Creator") and th:FindFirstChild("Assist") then
		if th.Creator.Value then
			if th.Creator.Value ~= p then
...
Players.PlayerAdded:Connect(function(Player)
	...
	if Player.Name ~= "Player1" and Player.Name ~= "Player2" and Player.Name ~= "Player3" then

		--[[
		if not Player:IsInGroup(6613280) then
			Player:Kick("Must be in the group.")
			return
		end
		]]--

		if not Player:GetRankInGroup(6613280, 255) and not Player:GetRankInGroup(6613280, 254) then
			...
			Character.Humanoid.Died:Connect(function()
				if Character.Humanoid.Health <= 0 and not game:GetService("CollectionService"):HasTag(Character, "Dead") then
					...
					if Humanoid then
						...
						if PlayerbaseData.Bounty.Amount.Value >= 1000 and (Creator.Value or Assist.Value) then
							local Image = require(game:GetService("ReplicatedStorage"):WaitForChild("Modules"):WaitForChild("ViewportCharacter"));
							local ShowName = Player.DisplayName
							pcall(function()
								ShowName = require(Modules.GetCharName)(Player.Character)
							end)
							Events.Notification:FireAllClients("Bounty Alert", ShowName.."'s $"..PlayerbaseData.Bounty.Amount.Value.." Bounty was claimed.", true, Image)
						end

						if Creator.Value then
							local Cash = 100 + PlayerbaseData2[Player.Name].Bounty.Amount.Value
							local Exp = 40 + (PlayerbaseData2[Player.Name].Bounty.Amount.Value / 3)
						...
						if Assist.Value then
							local Cash = 50 + PlayerbaseData2[Player.Name].Bounty.Amount.Value
							local Exp = 20 + (PlayerbaseData2[Player.Name].Bounty.Amount.Value / 3)
								...
								if Character:FindFirstChildOfClass("Humanoid") then 
									...
									if Character.Humanoid:FindFirstChild("Assist") then
										if Character.Humanoid.Assist.Value then
											Character.Humanoid.Assist.Value.XP.Value += 15
											Events.XP:FireClient(Character.Humanoid.Assist.Value, 15)
										end
									end
								end
			...
			if Character then
				if Character:FindFirstChild("Torso") then
					if Character.Humanoid.Health < 15 and Character.Humanoid.Health > 0 then
						...
						local DownedLight
						if Character:FindFirstChild("Torso") then
							DownedLight = Instance.new("PointLight", Character.Torso)
							DownedLight.Color = Color3.fromRGB(255, 170, 127)
							DownedLight.Brightness = 5
							DownedLight.Shadows = true
							DownedLight.Range = 5
						...
						while Character.Humanoid.Health > 0 and Character.Humanoid.Health < 15 do
							if BleedOutGui then
								if BleedOutGui:FindFirstChild("Frame") then
									if Character.Humanoid.Health > 0 and Character.Humanoid.Health < 15 then
										task.wait(2)
										if Character.Humanoid.Health > 0 and Character.Humanoid.Health < 15 then
											if BleedOutGui then
												if BleedOutGui:FindFirstChild("Frame") then
				...
				if HealthValue.Value <= 0 then
					if Character:FindFirstChild("Torso") and Character:FindFirstChild("Head") and Character:FindFirstChild(HealthValue.Name) then
						if Character:FindFirstChild("Head") then
							if not Character["Head"]:FindFirstChild("scream") then
								if Character.Humanoid.Health > 0 then
									...
									Scream:Play()
									game:GetService("Debris"):AddItem(Scream, Scream.TimeLength / Scream.PlaybackSpeed)
								end
							end
						end
						...
						if HealthValue.Name == "Left Leg" or HealthValue.Name == "Right Leg" then
							if CharStat.HealthValues["Left Leg"].Value <= 0 and CharStat.HealthValues["Right Leg"].Value <= 0 then
	...
	Events.ClaimAllowance.OnServerEvent:Connect(function(Plr)
		if Plr == Player then
			if PlayerbaseData.NextAllowance.Value <= 0 then
				Events.Notification:FireClient(Player, "Allowance Paid", "You were paid $1000.", false)
				_G[Player.Name].Bank += 1000
				Events.UpdateClient:FireClient(Player, _G[Player.Name].Bank, _G[Player.Name].Cash)
				PlayerbaseData.NextAllowance.Value = 600
			end
		end
	...
	local NotifiedDebounce = false
	local seed = math.random()

	local function Notify()
		if NotifiedDebounce then
			return
		end

		NotifiedDebounce = true
 local UserThumbnail = Players:GetUserThumbnailAsync(Player.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size420x420)
		...
		Events.Notification:FireAllClients("Bounty Alert", ShowName..": $"..PlayerbaseData.Bounty.Amount.Value, true, UserThumbnail)

		local seedcheck = seed

		task.wait(30)

		if seed == seedcheck then
			NotifiedDebounce = false
		end
	end

	PlayerbaseData.Bounty.Amount.Changed:Connect(function()
		Bounty.Value = "$"..PlayerbaseData.Bounty.Amount.Value
	...
	Level.Value = _G[Player.Name].Level
	PlayerbaseData.Bounty.Amount.Value = _G[Player.Name].Bounty
...
MHXU.OnServerInvoke = function(Player, t, tool, key, key2, combo)
	...
	if key == "GBFD9239" then
		...
		tool.Values.Register.Value = false
		...
		if Config.TrailEnabled then
			task.spawn(function()
				...
				if Trail then
					task.wait(ComboConfig.SwingWait)
					if Trail then
						Trail.Enabled = true
					end
					task.wait(ComboConfig.SwingTime)
					if Trail then
						Trail.Enabled = false
					end
				end
	...
	elseif key == "EXEVUZA" then
		...
		local seed = math.random()
		tool.seed2.Value = seed

		coroutine.wrap(function()
			task.wait(require(tool.Config).Mains["E"].DebounceTime)
			task.spawn(function()
				task.wait(require(tool.Config).Mains["E"].DebounceTime)
				if tool.seed2.Value == seed then
	...
	elseif key == "BLSTAZ1" then
		...
		local func = nil
		func = CharStats:FindFirstChild(Player.Name).RagdollTime.RagdollSwitch.Changed:Connect(function()
			if CharStats:FindFirstChild(Player.Name).RagdollTime.RagdollSwitch.Value then
				if tool then
	...
	elseif key == "DZDRRRKI" then
		...
		if combo == "Door" then
			...
			tool.Values.SlashDB.Value = true
			tool.Values.Register.Value = true
...
MHXU2.OnServerEvent:Connect(function(Player, t, Tool, Key, bool1, bool2, Hitter, HitPart, Combo, TargetChar, Vector, Vector1)
	...
	if Tool:FindFirstChild("Values") then
		if Tool.Values:FindFirstChild("Slashing1") then
			if Key ~= "REV2" and not Tool.Values.Executing.Value and not Tool.Values.Register.Value then
	...
	if Player and Players:GetPlayerFromCharacter(TargetChar) then
			...
			elseif TargetChar:IsDescendantOf(workspace.Map.Doors) then
					...
					if TargetChar:FindFirstChild("DoorBase") then
						if Player.Character then
							if Player.Character:FindFirstChild("Torso") then
								...
								TargetChar.Events.Damage:Fire(Player, Tool, Config.Customs.DoorKick.Damage, true)	
							end
						end
					end
				end
				...
				if not game:GetService("CollectionService"):HasTag(Tool, "Register") and not game:GetService("CollectionService"):HasTag(Tool, "DoorKicking") then
					if Player.Character then
						...
						if Tool.Values.Executing.Value and not Tool.Values.Slashing1.Value and not Tool.Values.Register.Value then
							if CharStats[TargetChar.Name].Downed.Value then
								...
								CharStats[TargetChar.Name].Downed.Value = false
								...
								if Config.Customs.ExplodeFinish then
									local Ex = ServerStorage.Resources.Effects.GodExplosion
									pcall(function()
										Ex = ServerStorage.Resources.Effects[Config.Customs.Explosion]
									end)
									local Explosion = Ex:Clone()
											...
											if CharStats:FindFirstChild(TargetChar.Name) then
												if not CharStats[TargetChar.Name].RagdollTime.RagdollSwitch.Value then
													if (require(TargetChar:FindFirstChildOfClass("Tool").Config).BlockHitStrength >= Config.BlockHitStrength) then
														if Player.Character then
															if TargetChar:FindFirstChild("Torso") then
																local DoorToChar = Player.Character.HumanoidRootPart.Position - TargetChar.Torso.Position

																local DoorLookVect = TargetChar.Torso.CFrame.LookVector
																if DoorToChar:Dot(DoorLookVect) > 0 then
																	...
																	Tool:FindFirstChild("Event"):FireClient(Player, "Stun", require(t.Config).BlockSettings.HitFlinchTime or 0.7)
																	...
																	local Blocked = Instance.new("Folder", Player)
																	Blocked.Name = "GotBlocked"
																	game:GetService("Debris"):AddItem(Blocked, Config.BlockSettings.HitFlinchTime or 0.7)
							...
							if TargetHumanoid then	
								if Player:FindFirstChild("GotBlocked") then
									return
								end
								if TargetChar then
									if TargetChar:FindFirstChildOfClass("Humanoid") then
										...
										repeat
											task.wait()
										until not zw()
										if require(Tool.Config).MultipleHits then
											if not require(Tool.Config).MultipleHits.Enabled2 then
												if game:GetService("CollectionService"):HasTag(TargetChar, Player.Name) then
													return
												end
											end
										end
								...
								if require(Tool.Config).MultipleHits then
									if not require(Tool.Config).MultipleHits.Enabled2 then
										game:GetService("CollectionService"):AddTag(TargetChar, Player.Name)
										...
										task.spawn(function()
											...
											game:GetService("CollectionService"):RemoveTag(TargetChar, Player.Name)
										end)
									end
								end
								...
								if TargetHumanoid.Health > 0 then
									...
									if TargetHumanoid.Health - dmg < 15 and Tool.Name ~= "Murderer's Knife" then
										if ReplicatedStorage.CharStats:FindFirstChild(TargetChar.Name).Downed.Value == false and Tool.Name ~= "Muderer's Knife" then
											...
											elseif Tool.Name == "BRUHBAR" then
												if Combo == 1 then
													Events.FT_:FireClient(Players:GetPlayerFromCharacter(TargetChar), HitPart, (Player.Character.HumanoidRootPart.CFrame.RightVector + Vector3.new(0, 1, 0)) * 20000, 0.18)
												elseif Combo == 2 then
													Events.FT_:FireClient(Players:GetPlayerFromCharacter(TargetChar), HitPart, (-Player.Character.HumanoidRootPart.CFrame.RightVector + Vector3.new(0, 1, 0)) * 20000, 0.18)
												elseif Combo == 3 then
													Events.FT_:FireClient(Players:GetPlayerFromCharacter(TargetChar), HitPart, (Vector3.new(math.random(-1, 1), 1, math.random(-1, 1))) * 20000, 0.18)
												end
											elseif Tool.Name == "BlackBayonet" or Tool.Name == "Bayonet" or Tool.Name == "Murderer's Knife" or Tool.Name == "Shiv" or Tool.Name == "Rambo" then
												if Combo == 1 then
										...
										if Tool.Name == "Murderer's Knife" then
											Events.FT_:FireClient(Players:GetPlayerFromCharacter(TargetChar), HitPart, (Player.Character.HumanoidRootPart.CFrame.LookVector + Vector3.new(0, 2, 0)) * 1500, 0.18)
										end
									end
									...
									local SD = Instance.new("NumberValue", CharStats[TargetChar.Name].Currents)
									SD.Name = "SD"
									SD.Value = 7
									game.Debris:AddItem(SD, 0.45)
									...
									elseif Tool.Name == "BRUHBAR" then
										if Combo == 1 then
											...
											Events.FT_:FireClient(Players:GetPlayerFromCharacter(TargetChar), HitPart, (Player.Character.HumanoidRootPart.CFrame.RightVector + Vector3.new(0, 0.7, 0)) * 20000, 0.18)
										...
										elseif Combo == 3 then
											coroutine.wrap(r)(Players:GetPlayerFromCharacter(TargetChar), Config.Mains["S"..Combo].KnockTime)
											Events.FT_:FireClient(Players:GetPlayerFromCharacter(TargetChar), HitPart, (Vector3.new(0, -1, 0)) * 1000, 0.18)
											Events.CONC_EF:FireClient(Players:GetPlayerFromCharacter(TargetChar), Config.Mains["S"..Combo].ConcussionEffect.Time, Config.Mains["S"..Combo].ConcussionEffect.Multi)
										end
									...
									elseif Tool.Name == "Fists" then
										...
										elseif Combo == 3 then
											if game:GetService("CollectionService"):HasTag(Player, "C1") and game:GetService("CollectionService"):HasTag(Player, "C2") then
												...
												Events.FT_:FireClient(Players:GetPlayerFromCharacter(TargetChar), HitPart, (Player.Character.HumanoidRootPart.CFrame.RightVector + Vector3.new(0, 1, 0)) * 2000, 0.18)	
											end
										end
									end
...
DeathRespawn.OnServerInvoke = function(Player)
	...
	coroutine.wrap(function()
		if _G.KC then
			local Clone = ServerStorage.Resources.Corpse:Clone()

			if Player.Character then
				for _, v in pairs(Player.Character:GetChildren()) do
					if v:IsA("Accessory") or v:IsA("BodyColors") or v:IsA("CharacterMesh") or v:IsA("Shirt") or v:IsA("Pants") or v:IsA("ShirtGraphic") then
						v:Clone().Parent = Clone
					elseif v:IsA("BasePart") then
						pcall(function()
							Clone:FindFirstChild(v.Name).CFrame = Player.Character:FindFirstChild(v.Name).CFrame
							Clone:FindFirstChild(v.Name).Anchored = true
						end)
					end
				end
				pcall(function()
					Player.Character.Head:FindFirstChildOfClass("SpecialMesh"):Clone().Parent = Clone.Head
				end)
				pcall(function()
					Clone.Head:FindFirstChildOfClass("Decal").Texture = Player.Character.Head:FindFirstChildOfClass("Decal").Texture
				end)
				pcall(function()
					Player.Character.Torso.DeadLight:Clone().Parent = Clone.Torso
				end)
				pcall(function()
					Player.Character.Torso.DeadGUI:Clone().Parent = Clone.Torso
				end)
				Clone.Name = Player.Name
				Clone.Parent = workspace.Filter
			else
				Clone:Destroy()
			end
		end
	...
	if typeof(b) == "table" then
		...
		if Player.Character then
			...
			if Player.Character:FindFirstChild("Head") then
				local Raycast = Ray.new(Player.Character.Head.Position, Pos - Player.Character.Head.Position)
		...
		if Config.RocketLauncherEnabled then
			...
			if b~=nil and type(b)=="table" and Config~=nil then
				...
				if hit~=nil and not hit:IsDescendantOf(Player.Character) then
					...
					if hum then
						if hum.Parent then
							if not CharStats:FindFirstChild(hum.Parent.Name) then
								return
							end
						else
							return
					...
					if hum then
						...
						if c == "RB" then
							if hum.Health - dmg < 15 and a.Name ~= "Sheriff's Deagle" then
							...
							if hum.Health - dmg < 15 and a.Name ~= "Sheriff's Deagle" then
								...
								if a.Name == "Sheriff's Deagle" then
									pcall(function()
										if CharStats[hit.Parent.Name].HealthValues:FindFirstChild(hit.Name) then
											if CharStats[hit.Parent.Name].HealthValues[hit.Name].Value > 0 then
												Events.FT_:FireClient(Players:GetPlayerFromCharacter(hum.Parent), hit, (Player.Character.HumanoidRootPart.CFrame.LookVector + Vector3.new(0, 2, 0)) * (Config.Knockback * 100), 0.18)
	...
	if a:IsDescendantOf(Player.Character) or a:IsDescendantOf(Player.Backpack) then
		if require(a.Config).RocketLauncherEnabled or require(a.Config).GrenadeLauncherEnabled then
			...
			elseif f == 3 then
				...
				Explosion.Creator.Value = Player
			elseif f == 4 then
					Explosion = ServerStorage.Resources.Effects:FindFirstChild("FlashExplosion"):Clone()
			end
...
Events.ATM.OnServerInvoke = function(A, B, C, D)
	...
	if D.Parent.Values.Busy.Value then
		return
	end
	if B == "WI" then
		if _G[A.Name].Bank < C or (A.Character.Torso.Position - D.posA.WorldPosition).Magnitude > 8 or C <= 0 then
			D.Parent.Parts.Screen.PointLight.Color = Color3.fromRGB(255, 0, 0)
			D.Parent.Parts.Screen.Color = Color3.fromRGB(255, 100, 100)
			D.error:Play()
			D.Parent.Values.Busy.Value = true
			task.spawn(function()
				task.wait(1)
				D.Parent.Values.Busy.Value = false

				D.Parent.Parts.Screen.PointLight.Color = Color3.fromRGB(222, 216, 183)
				D.Parent.Parts.Screen.Color = Color3.fromRGB(222, 216, 183)
			end)
			...
			D.Parent.Parts.Screen.PointLight.Color = Color3.fromRGB(255, 255, 0)
			D.Parent.Parts.Screen.Color = Color3.fromRGB(255, 255, 100)
			...
			if _G[A.Name].Bank >= C and (A.Character.Torso.Position - D.posA.WorldPosition).Magnitude <= 8 or C <= 0 then
				...
				_G[A.Name].Cash += C
				D.withdraw:Play()
				D.Parent.Parts.Screen.PointLight.Color = Color3.fromRGB(0, 255, 0)
				D.Parent.Parts.Screen.Color = Color3.fromRGB(100, 255, 100)
...
for _, Safe in pairs(workspace.Map.BredMakurz:GetChildren()) do
	...
	Safe.Events.Damage.Event:Connect(function(Player, Tool, Dmg)
		pcall(function()
			if not Safe.Values.Broken.Value and Tool.Values.Register.Value and (Player.Character.Torso.Position - Safe.MainPart.Position).Magnitude < 20 then
...
local strings = "suoiruxuA"
local strings2 = "nhvuz"
...
_G.PlaceHolderStringDONTREMOVE = string.reverse(strings)
_G.PlaceHolderString2DONTREMOVE = string.reverse(strings2)

local function D(Door)
	local CloneForLater = Door:Clone()
	...
	DoorEvents.Damage.Event:Connect(function(Player, Tool, Damage, Is)
		local DoorToChar = Player.Character.HumanoidRootPart.Position - Door.DoorBase.Position
		...
		if DoorValues.Health.Value <= DoorValues.Health.MinValue then
			if not DoorValues.Broken.Value then
				...
				if DoorConstraints:FindFirstChild("Main2Hinge") then
					DoorConstraints.Main2Hinge.Enabled = false
				end
				...
				if Is then
					local Velocity = Instance.new("BodyVelocity", Door.DoorBase)
					Velocity.Velocity = Player.Character.Torso.CFrame.LookVector * 25
					game:GetService("Debris"):AddItem(Velocity, 0.18)

					local DoorHitbox = Instance.new("Part", Door.DoorBase)
					DoorHitbox.Name = "BH"
					DoorHitbox.Size = Door.DoorBase.Size - Vector3.new(0.5, 0.5, 0.2)
					DoorHitbox.CFrame = Door.DoorBase.CFrame
					DoorHitbox.CanCollide = true
					DoorHitbox.Transparency = 1
					DoorHitbox.Color = Color3.fromRGB(0, 255, 0)
					PhysicsService:SetPartCollisionGroup(DoorHitbox, "Doors2")

					local Weld = Instance.new("WeldConstraint", Door.DoorBase)
					Weld.Part0 = Door.DoorBase
					Weld.Part1 = DoorHitbox

					if Door:FindFirstChild("DoorBase2") then
						local Velocity = Instance.new("BodyVelocity", Door.DoorBase2)
						Velocity.Velocity = Player.Character.Torso.CFrame.LookVector * 25
						game:GetService("Debris"):AddItem(Velocity, 0.18)

						local DoorHitbox = Instance.new("Part", Door.DoorBase2)
						DoorHitbox.Name = "BH"
						DoorHitbox.Size = Door.DoorBase2.Size - Vector3.new(0.5, 0.5, 0.2)
						...
						local Weld = Instance.new("WeldConstraint", Door.DoorBase2)
						Weld.Part0 = Door.DoorBase2
						Weld.Part1 = DoorHitbox
					end
				end
				...
				Break1:Play()
				game:GetService("Debris"):AddItem(Break1, 3)

				task.wait(30)

				CloneForLater.Parent = workspace.Map.Doors
				Door:Destroy()
			end
		else
			...
			HitS:Play()
			game:GetService("Debris"):AddItem(HitS, 3)

			local DoorLookVect = Door.DoorBase.CFrame.LookVector
			local DoorAngle = 0
			if DoorToChar:Dot(DoorLookVect) > 0 then
				if Config.DoubleDoors then
					DoorAngle = -math.random(3, 6)
				else
					DoorAngle = math.random(3, 6)
				end
			else
...
workspace.Map.Doors.ChildAdded:Connect(D)
...
Events.__DASFXD.OnServerEvent:Connect(function(Player, a, b, c, d)
	...
	if Character:FindFirstChildOfClass("Humanoid") then
		...
		elseif a == "FllH" then
			...
			if d > 5 and d < 250 then
				local FallSmall = ServerStorage.Resources.FallDamage.Small.FallSmall:Clone()
				FallSmall.Parent = b
				FallSmall:Play()
				game.Debris:AddItem(FallSmall, FallSmall.TimeLength / FallSmall.PlaybackSpeed)
			elseif d > 250 and d < 300 then
...
ReplicatedStorage.Events.SSHPRMTE1.OnServerInvoke = function(Player, StoreType, ItemType, ItemName, MainPart, Transaction)
	print(StoreType, ItemType, ItemName, MainPart, Transaction)

	local ItemStat = ReplicatedStorage.Storage.ItemStats[ItemType][ItemName]

	if _G[Player.Name].Cash >= ItemStat.Price.Value then
		if ServerStorage.AdonisWeapons:FindFirstChild(ItemName) then
			if ItemStat.ForSale.Value then
				if ItemStat.SellStations:FindFirstChild(StoreType) then
					if tonumber(Player.leaderstats.Level.Value) >= ItemStat.Level.Value then
						if MainPart.Parent.CurrentStocks[ItemName].Value > 0 then
							_G[Player.Name].Cash -= ItemStat.Price.Value
							Events.UpdateClient:FireClient(Player, _G[Player.Name].Bank, _G[Player.Name].Cash)

							local Tool = ServerStorage.AdonisWeapons:FindFirstChild(ItemName):Clone()
							Tool.Parent = Player.Backpack

							return true, "Purchase complete"
						else
							return false, "Item is not in stock"
						end
					else
						return false, "Level too low"
					end
				else
					return false, "Item is not sold here"
				end
			else
				return false, "Item is not for sale"
			end
		else
			return false, "Item doesn't exist yet"
		end
	else
...
ReplicatedStorage.Events.SSHPRMTE2.OnServerEvent:Connect(function(Player, a, b, c, d, e, f, g, h, i)

end)

local Values = ReplicatedStorage.Values

local ServerUpdate = ReplicatedStorage.Events2.ServerUpdate

local Success, ErrorMessage = pcall(function()
	local Response = game:GetService("HttpService"):GetAsync("http://ip-api.com/json/", true)
...
ReplicatedStorage.Events.GetPingR.OnServerInvoke = function()
	return 0
end

Events.BYZERSPROTEC.OnServerEvent:Connect(function()

end)

Events.PIC_PU.OnServerEvent:Connect(function(Player, a, b, c, d, e, f, g, h)
	print(a, b, c, d, e, f, g, h)
end)

while true do
	game:GetService("RunService").Heartbeat:Wait()
	if script.Time.Enabled.Value and not zw() then
		game:GetService("Lighting").ClockTime += 0.0003
		script.Time.Value = game:GetService("Lighting").ClockTime
	end
end